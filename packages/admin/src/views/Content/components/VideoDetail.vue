          ...
          <el-tab-pane label="基本信息" name="base">
            <div class="video-base-info">
              <div class="base-info-item">
                ...
                <div class="item-value">{{ videoInfo.name }}</div>
              </div>
              <div class="base-info-item">
...
const videoInfo = ref<VideoPostVO>({} as VideoPostVO);
const videoFileList = ref<VideoFilePost[]>([]);
//播放器
const playerRef = ref();
//当前播放的视频
...
const loadPList = async () => {
  let result = await VideoService.loadVideoPList(videoInfo.value.id);
  if (!result) {
    return;
  }
  videoFileList.value = result;
  await nextTick(() => {
